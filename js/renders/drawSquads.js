  ...
  for (let squad in squads) {
    if (squads[squad].state === 1) {
      ...
      if (squads[squad].player === 2) {
        ...
        q.strokeStyle = "rgba(77,77,77,0.9)"
        q.beginPath();
        q.moveTo(cur.left + cur.width - 6, cur.top + cur.height + 12)
        q.lineTo(cur.left + cur.width + 12,cur.top + cur.height -5)
        q.stroke();
        q.lineWidth = 2;
        ...
        // Число юнитов в отряде и рамочка для них
        q.fillStyle = "rgba(187,185,185,1)";
        q.fillRect(cur.left + cur.width, cur.top, 16, 16);
        q.strokeRect(cur.left + cur.width, cur.top, 16, 16);
        q.fillStyle = "black"
        if (cur.count - cur.deads < 10) {
          q.font = "14px arial";
          q.fillText(`${(cur.count - cur.deads) || 0}`, cur.left + cur.width + 4, cur.top + 12.5);
        } else {
          q.font = "12px arial";
          q.fillText(`${(cur.count - cur.deads) || 0}`, cur.left + cur.width + 1, cur.top + 11.6);
        }

        // Номер бинда отряда
        q.fillStyle = "rgba(111,111,185,1)";
        q.fillRect(cur.left + cur.width, cur.top + 16, 16, 16);
        q.strokeRect(cur.left + cur.width, cur.top + 16, 16, 16);
        q.fillStyle = "black"
        if (cur.keyBind === '') {
          q.fillText(`x`, cur.left + cur.width + 4, cur.top + 29);
        } else {
          q.fillText(`${cur.keyBind.slice(-1)}`, cur.left + cur.width + 4, cur.top + 29);
        }
      }
